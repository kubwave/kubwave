...
// claimDueWatchServices SKIP-LOCKED claims due docker-image services in a tx, then leases each claimed row forward.

let dueRows: Array<{ id: string }> = [];
const updates: Array<{ id: string; set: Record<string, unknown> }> = [];

mock.module('~/shared/config/worker-env', () => ({ env: { registryTagWatchServiceIntervalSeconds: 120 } }));
...
mock.module('@kubwave/db', () => ({
	services: { id: 'id', type: 'type', config: 'config', imageWatchEnabled: 'iwe', nextWatchAt: 'nw', lastWatchedDigest: 'lwd' },
	db: {
...
const { claimDueWatchServices } = await import('~/modules/worker/jobs/registry-tag-watch/claim');
...
describe('claimDueWatchServices', () => {
	test('returns the claimed services and leases each one forward', async () => {
		dueRows = [{ id: 'svc-a' }, { id: 'svc-b' }];
		const rows = await claimDueWatchServices(now, 20);
		expect(rows).toEqual(dueRows as typeof rows);
		expect(updates.map(u => u.id)).toEqual(['svc-a', 'svc-b']);
		for (const u of updates) {
			expect(u.set.lastWatchedAt).toBe(now);
			expect(u.set.nextWatchAt).toBeInstanceOf(Date);
			expect((u.set.nextWatchAt as Date).getTime()).toBeGreaterThan(now.getTime());
		}
	});
