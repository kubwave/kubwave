...
const schema = z.object({
	...
	installationId: z.string().min(1, 'Select a Gitea account.'),
...
const { data: installations, isPending: installationsPending } = useGiteaInstallations(activeTeamId);
const { data: repos, isPending: reposPending } = useGiteaRepos(activeTeamId, selectedInstallationId);
const syncRepos = useSyncGiteaRepos(activeTeamId, selectedInstallationId);
...
<template>
	<div v-if="noInstallations" class="flex flex-col gap-4">
		<div class="rounded-lg border border-dashed px-4 py-6 text-sm text-muted-foreground">
			No Gitea repositories are available yet. Connect a Gitea account in
			<NuxtLink to="/team/settings?tab=gitea" class="font-medium text-foreground underline">team settings → Gitea</NuxtLink>.
		</div>
		<div>
	...
	<AppForm v-else :form="form" class="flex flex-col gap-4">
		...
		<p v-if="selectedInstallationId" class="-mt-2 flex flex-wrap items-center gap-1.5 text-xs text-muted-foreground">
			Just granted access to a repository and don't see it?
			<button
				type="button"
			...
			>
				...
				{{ syncRepos.isPending.value ? 'Syncing…' : 'Refresh from Gitea' }}
			</button>
		</p>
