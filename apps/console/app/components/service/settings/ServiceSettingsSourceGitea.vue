	...
	<section class="flex flex-col gap-5">
		<div>
			...
			<p class="text-xs text-muted-foreground">
				A Gitea repository deployed through a connected Gitea account. Change the branch or build here; to move to a different repository, recreate
				the service.
			</p>
		</div>
		...
		<div class="flex flex-col gap-3">
			...
			<div class="grid gap-4 sm:grid-cols-6">
				<div class="flex flex-col gap-1.5 sm:col-span-4">
					...
					<div class="rounded-md border bg-muted/30 px-3 py-2 font-mono text-xs">{{ state.repoFullName }}</div>
				</div>
	...
	<section class="flex flex-col gap-3">
		<div class="flex items-start justify-between gap-2">
			<div>
				...
				<p class="text-xs text-muted-foreground">Deploy automatically when a new commit lands — via webhook, with polling as a fallback.</p>
			</div>
